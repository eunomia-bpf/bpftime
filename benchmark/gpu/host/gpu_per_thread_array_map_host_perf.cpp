...
static bench_options parse_args(int argc, char **argv)
{
	...
	for (int i = 1; i < argc; ++i) {
		if (parse_u64_arg(argv[i], "iters", opt.iters) ||
		    parse_u64_arg(argv[i], "max-entries",
				  opt.max_entries) ||
		    parse_u64_arg(argv[i], "value-size",
				  opt.value_size) ||
		    parse_u64_arg(argv[i], "thread-count",
				  opt.thread_count) ||
		...
		if (std::strcmp(argv[i], "--thread-count") == 0 &&
		    i + 1 < argc) {
			opt.thread_count =
				std::strtoull(argv[++i], nullptr, 10);
			continue;
...
int main(int argc, char **argv)
{
	...
	attr.type = (int)bpftime::bpf_map_type::
		BPF_MAP_TYPE_PERGPUTD_ARRAY_MAP;
	attr.key_size = sizeof(uint32_t);
	attr.value_size = (uint32_t)opt.value_size;
	attr.max_ents = (uint32_t)opt.max_entries;
	attr.gpu_thread_count = opt.thread_count;

	int map_fd = bpftime_maps_create(
		-1, "gpu_per_thread_array_map_host_perf", attr);
	if (map_fd < 0) {
	...
	const uint64_t entry_size =
		opt.value_size * opt.thread_count;
	std::vector<uint8_t> value(entry_size, 0);

	// Pre-fill the map with some values to avoid ENOENT on lookups.
	for (uint32_t i = 0; i < opt.max_entries; ++i) {
		uint32_t key = i;
		if (bpftime_map_update_elem(map_fd, &key, value.data(),
					    0) != 0) {
			std::perror(
				"bpftime_map_update_elem (prefill PERGPUTD)");
			return 1;
		}
	}
