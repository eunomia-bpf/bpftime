...
nv_attach_impl::extract_ptxs(std::vector<uint8_t> &&data_vec)
{
	...
	const std::string cuobjdump = cuobjdump_path->string();
...
nv_attach_impl::hack_fatbin(std::map<std::string, std::string> all_ptx)
{
	...
	*/
	...
	if (spdlog::should_log(spdlog::level::debug)) {
		char tmp_dir[] = "/tmp/bpftime-fatbin-work.XXXXXX";
		if (mkdtemp(tmp_dir) == nullptr) {
			// PTX patching already succeeded; only the optional
			// debug dump dir could not be created, so keep the
			// patched result rather than failing hack_fatbin.
			SPDLOG_WARN("mkdtemp failed for {}: {}", tmp_dir,
				    strerror(errno));
			return ptx_out;
		}
		auto working_dir = std::filesystem::path(tmp_dir);
