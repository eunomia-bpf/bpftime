...
TEST_CASE("GPU verifier rejection has a dedicated attach status")
{
	REQUIRE(should_propagate_gpu_verifier_error(BPFTIME_VERIFIER_STRICT,
						    GPU_VERIFIER_REJECTED));
	REQUIRE_FALSE(should_propagate_gpu_verifier_error(
		BPFTIME_VERIFIER_WARNING, GPU_VERIFIER_REJECTED));
	REQUIRE_FALSE(should_propagate_gpu_verifier_error(
		BPFTIME_NO_VERIFY, GPU_VERIFIER_REJECTED));
	REQUIRE_FALSE(should_propagate_gpu_verifier_error(
		BPFTIME_VERIFIER_STRICT, -EINVAL));
}
