...
bool is_memory_op(const ebpf_inst &instruction)
{
	...
	return cls == INST_CLS_LDX || cls == INST_CLS_ST || cls == INST_CLS_STX;
}
...
ResourceBudgetResult check_resource_budget(const ebpf_inst *instructions,
					   ...
					   const ebpf_verifier_stats_t *stats)
{
	...
	// memory-style counts by the same factor as an approximation. This
	// keeps loop accounting honest without pretending we have per-helper
	// loop bounds.
	const uint64_t loop_multiplier = std::max<uint64_t>(
		1, ceiling_divide(
			   worst_case_instruction_count,
			   std::max<uint64_t>(1, static_instruction_count)));

	result.instruction_count =
		saturating_cast(worst_case_instruction_count);
