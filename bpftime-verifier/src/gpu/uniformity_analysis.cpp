...
{
	return width > 0 && offset >= -STACK_SIZE && offset <= -1 &&
	       width <= static_cast<size_t>(-offset);
}

std::optional<int32_t> add_constant_offset(int32_t offset, int64_t delta)
{
	const int64_t result = static_cast<int64_t>(offset) + delta;
				     ...
				     size_t width)
{
	for (auto it = state.stack_pointer_slots.begin();
		...
		const int64_t slot_offset = it->first;
		const bool overlaps =
			slot_offset < static_cast<int64_t>(offset) +
...
void write_stack_uniformity(UniformityState &state, int32_t offset,
			    size_t width, Uniformity value)
{
				 ...
				 size_t width)
{
	...
	for (size_t i = 0; i < width; ++i) {
		result = bpftime::verifier::gpu::join_uniformity(
			result, state.stack_bytes[stack_index(
			       ...
			       const PointerProvenance &rhs)
{
	...
	result.region =
		lhs.region == rhs.region ? lhs.region : PointerRegion::UNKNOWN;
...
{
	if (map_fd.has_value()) {
		if (auto it = maps.find(*map_fd);
		    it != maps.end() && it->second.key_size > 0) {
			 ...
			 const std::map<int, BpftimeMapDescriptor> &maps)
{
	...
	if (is_alu_imm(instruction)) {
		...
		if (is_add_sub(instruction) &&
			...
			if (output.pointers[dst].constant_offset.has_value()) {
				const int64_t delta =
					(instruction.opcode &
					 INST_ALU_OP_MASK) == INST_ALU_OP_SUB ?
						-static_cast<int64_t>(
							instruction.imm) :
						instruction.imm;
				output.pointers[dst]
					.constant_offset = add_constant_offset(
	...
	if ((instruction.opcode & INST_CLS_MASK) == INST_CLS_LDX) {
		...
		if (base_pointer.region == PointerRegion::STACK) {
			if (base_pointer.offset_uniformity ==
			    Uniformity::VARYING) {
			...
			const auto offset = add_constant_offset(
			...
			output.regs[dst] =
				load_stack_uniformity(input, *offset, width);
			output.pointers[dst] = {};
			if (width == 8) {
				if (auto it = input.stack_pointer_slots.find(
					    *offset);
				    ...
				    size_t width)
{
	...
	if (pointer.region == PointerRegion::STACK) {
		...
		return query_stack_uniformity(state, *pointer.constant_offset,
					      width);
	}
...
size_t infer_pointer_access_width(const UniformityState &state, uint8_t reg)
{
	...
	if (pointer.region == PointerRegion::STACK &&
	    pointer.offset_uniformity == Uniformity::UNIFORM &&
	    pointer.constant_offset.has_value()) {
		return std::max<size_t>(1, contiguous_stack_region_width(
						   state,
...
analyze_uniformity(const ebpf_inst *instructions, size_t num_instructions,
		   const std::map<int, BpftimeMapDescriptor> &maps)
{
