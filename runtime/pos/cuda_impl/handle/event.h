...
class POSHandle_CUDA_Event final : public POSHandle_CUDA {
 public:
     ...
     */
    std::string get_resource_name() override{ return std::string("CUDA Event"); }
...
class POSHandleManager_CUDA_Event : public POSHandleManager<POSHandle_CUDA_Event> {
 ...
 private:
     ...
     *  \return POS_SUCCESS for successfully restore
     */
    pos_retval_t __reallocate_single_handle(void* mapped, uint64_t ckpt_file_size, POSHandle_CUDA_Event** handle) override;
};
