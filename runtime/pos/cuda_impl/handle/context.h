...
class POSHandle_CUDA_Context final : public POSHandle_CUDA {
 public:
     ...
     */
    POSHandle_CUDA_Context(void* hm);
     ...
     */
    std::string get_resource_name()override{ return std::string("CUDA Context"); }
 ...
 protected:
    friend class POSHandleManager_CUDA_Context;
    friend class POSHandleManager<POSHandle_CUDA_Context>;
...
class POSHandleManager_CUDA_Context : public POSHandleManager<POSHandle_CUDA_Context> {
 ...
 private:
     ...
     *  \return POS_SUCCESS for successfully restore
     */
    pos_retval_t __reallocate_single_handle(void* mapped, uint64_t ckpt_file_size, POSHandle_CUDA_Context** handle) override;
};
