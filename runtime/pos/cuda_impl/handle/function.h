...
class POSHandle_CUDA_Function final : public POSHandle_CUDA {
 public:
     ...
     */
    std::string get_resource_name()override{ return std::string("CUDA Function"); }
...
class POSHandleManager_CUDA_Function : public POSHandleManager<POSHandle_CUDA_Function> {
 ...
 private:
     ...
     *  \return POS_SUCCESS for successfully restore
     */
    pos_retval_t __reallocate_single_handle(void* mapped, uint64_t ckpt_file_size, POSHandle_CUDA_Function** handle) override;
};
