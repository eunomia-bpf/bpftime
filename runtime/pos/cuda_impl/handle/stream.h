 ...
 */
class POSHandle_CUDA_Stream final : public POSHandle_CUDA {
 public:
    /*!
     *  \brief  constructor
     ...
     */
    std::string get_resource_name()override{ return std::string("CUDA Stream"); }
...
class POSHandleManager_CUDA_Stream : public POSHandleManager<POSHandle_CUDA_Stream> {
 ...
 private:
     ...
     *  \return POS_SUCCESS for successfully restore
     */
    pos_retval_t __reallocate_single_handle(void* mapped, uint64_t ckpt_file_size, POSHandle_CUDA_Stream** handle) override;
};
