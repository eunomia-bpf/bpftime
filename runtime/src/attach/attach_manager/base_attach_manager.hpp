...
class base_attach_manager {
	...
	using filter_callback =
		std::function<bool(const pt_regs &regs)>;
	using callback_variant =
		std::variant<uprobe_callback, uretprobe_callback,
			     replace_callback, filter_callback>;
	using attach_iterate_callback = std::function<void(int id, const void *addr,attach_type ty)>;

	virtual ~base_attach_manager();

	virtual const void *
	resolve_function_addr(const std::string_view &module_name,
