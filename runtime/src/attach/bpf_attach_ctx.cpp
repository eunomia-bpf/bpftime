				...
				if (int err = instantiate_handler_at(
					...
					if (err ==
						...
						const int cleanup_error =
							destroy_all_attach_links_unlocked();
						const auto cleanup = detail::
							decide_rejected_attach_cleanup(
								err, cleanup_error);
						if (!cleanup.reset_state) {
							SPDLOG_ERROR(
								"Unable to clean up after GPU verifier rejection: {}; preserving remaining attach tracking",
								cleanup_error);
							return cleanup.error_to_return;
						}
						reset_instantiated_state_unlocked();
						return cleanup.error_to_return;
					}
