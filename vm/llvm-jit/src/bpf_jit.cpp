...
bpf_jit_context::bpf_jit_context(const ebpf_vm *m_vm) : vm(m_vm)
{
	using namespace llvm;
	int zero = 0;
...
ebpf_jit_fn bpf_jit_context::compile()
{
	...
	if (!this->jit.has_value()) {
		...
		const auto tryDefineLddwHelper = [&](const char *name,
						     void *func) {
			if (func) {
				SPDLOG_DEBUG(
					"Defining LDDW helper {} with addr {:x}",
					name, (uintptr_t)func);
				auto sym =
		...
		tryDefineLddwHelper(LDDW_HELPER_MAP_BY_FD,
				    (void *)vm->map_by_fd);
		tryDefineLddwHelper(LDDW_HELPER_MAP_BY_IDX,
				    (void *)vm->map_by_idx);
		tryDefineLddwHelper(LDDW_HELPER_MAP_VAL, (void *)vm->map_val);
		tryDefineLddwHelper(LDDW_HELPER_CODE_ADDR,
				    (void *)vm->code_addr);
...
bpf_jit_context::~bpf_jit_context()
{
